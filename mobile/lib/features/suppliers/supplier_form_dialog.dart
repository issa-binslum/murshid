...
class _SupplierFormDialogState extends ConsumerState<SupplierFormDialog> {
  ...
  Future<void> _submit({bool addAnother = false}) async {
    ...
    bool ok;
    if (_isEdit) {
      ok = await ref.read(supplierProvider.notifier).update(
            widget.editing!.id,
      ...
      ok = await ref.read(supplierProvider.notifier).create(
            name: _name.text.trim(),
            phone: _phone.text.trim(),
            email: _email.text.trim(),
            address: _address.text.trim(),
    ...
    if (!mounted) return;
    if (!ok) {
      final err = ref.read(supplierProvider).error ?? 'An error occurred';
      ref.read(supplierProvider.notifier).clearError();
      setState(() {
        _saving = false;
        _error = err;
      });
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _Field(
                          label: 'Address',
                          controller: _address,
                          hint: 'e.g. P.O. Box 456, Dar es Salaam',
                          maxLines: 2,
                        ),
                        const SizedBox(height: 14),
                        // Credit Limit
