...
import '../../core/utils/sales_invoice_pdf.dart';
import '../../core/widgets/money_text.dart';
import '../receipts/receipt_form_page.dart';
...
class SalesInvoiceDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final live = ref.watch(salesInvoiceProvider).items.firstWhere(
          (i) => i.id == invoice.id,
          orElse: () => invoice,
        );
        ...
        child: Column(
          ...
          children: [
            _PageHeader(
              ...
              onPdf: () async {
                final bytes = await buildSalesInvoicePdf(live);
                if (context.mounted) {
                  await saveAndOpenPdf(
                      context, bytes, 'SalesInvoice_$shortId.pdf');
                }
              },
              onEdit: () => Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => SalesInvoiceFormPage(editing: live),
                  fullscreenDialog: true,
                ),
              ),
              onCopyToReceipt: (live.isCancelled || live.status == 'PAID')
                  ? null
                  : () => Navigator.of(context).push(
                        MaterialPageRoute(
                          builder: (_) => ReceiptFormPage(fromInvoice: live),
                          fullscreenDialog: true,
                        ),
                      ),
              onDuplicateInvoice: () => Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => SalesInvoiceFormPage(fromInvoice: live),
                  fullscreenDialog: true,
                ),
              ),
  ...
  void _showMarkPaidDialog(
      BuildContext context, WidgetRef ref, SalesInvoice inv) {
    showDialog<void>(
      context: context,
...
class _PageHeader extends StatelessWidget {
  ...
  final VoidCallback? onCopyToReceipt;
...
class _CopyToBtn extends StatelessWidget {
  final VoidCallback? onCopyToReceipt;
  final VoidCallback onDuplicateInvoice;
  const _CopyToBtn({this.onCopyToReceipt, required this.onDuplicateInvoice});
  ...
  Widget build(BuildContext context) {
    return PopupMenuButton<String>(
      ...
      onSelected: (value) {
        if (value == 'receipt') onCopyToReceipt?.call();
        if (value == 'duplicate') onDuplicateInvoice();
      },
      itemBuilder: (_) => [
        if (onCopyToReceipt != null)
          const PopupMenuItem(
            value: 'receipt',
            child: Row(
              children: [
                Icon(Icons.receipt_outlined, size: 16, color: AppColors.textPrimary),
                SizedBox(width: 10),
                Text('Receipt', style: TextStyle(fontSize: 13, color: AppColors.textPrimary)),
              ],
            ),
          ),
        const PopupMenuItem(
...
class _MarkPaidDialogState extends ConsumerState<_MarkPaidDialog> {
  ...
  Future<void> _confirm() async {
    setState(() => _busy = true);
    final ok =
        await ref.read(salesInvoiceProvider.notifier).markPaid(widget.invoice.id);
    if (!mounted) return;
    if (ok) {
            ...
            children: [
              ...
              RichText(
                text: TextSpan(
                  ...
                  children: [
                    const TextSpan(
                        text: 'You are about to cancel invoice '),
                    TextSpan(
                      text: '#$shortId',
                      style: const TextStyle(
                          fontWeight: FontWeight.w700,
