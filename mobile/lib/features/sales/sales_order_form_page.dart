...
class SalesOrderFormPage extends ConsumerStatefulWidget {
  final SalesOrder? editing;
  final SalesOrder? fromOrder;
  const SalesOrderFormPage({super.key, this.editing, this.fromOrder});

  @override
  ConsumerState<SalesOrderFormPage> createState() =>
      _SalesOrderFormPageState();
}
...
class _SalesOrderFormPageState extends ConsumerState<SalesOrderFormPage> {
  ...
  void initState() {
    ...
    _customerId = e?.customerId ?? fo?.customerId;
    _addressCtrl.text = e?.address ?? fo?.address ?? '';
    _descCtrl.text = e?.description ?? fo?.description ?? '';
  ...
  // ── Submit: create and reset for another ───────────────────────────────────
  ...
  Future<void> _submitAndAddAnother() async {
    ...
    if (ok) {
      // Reset form state — keep page open
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Order created. Ready for next entry.',
  ...
  Future<bool> _persist() async {
    if (_isEdit) {
      return ref.read(salesOrderProvider.notifier).update(
            widget.editing!.id,
      ...
      return ref.read(salesOrderProvider.notifier).create(
            customerId: _customerId!,
            date: _date,
            items: _linePayload,
            address: _addressCtrl.text.trim(),
  ...
  void _handleError() {
    final err = ref.read(salesOrderProvider).error ?? 'An error occurred.';
    ref.read(salesOrderProvider.notifier).clearError();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: isWide ? _wideLayout(customers, inventory) : _narrowLayout(customers, inventory),
    );
  }
  ...
  Widget _narrowLayout(List<Customer> customers, List<InventoryItem> inventory) {
    return Column(
      children: [
        _Header(
          title: _isEdit ? 'Edit Sales Order' : 'New Sales Order',
  ...
  Widget _cardBody(List<Customer> customers, List<InventoryItem> inventory) {
    return Column(
  ...
  Widget _formContent(List<Customer> customers, List<InventoryItem> inventory) {
      ...
      child: Column(
        ...
        children: [
          ...
          TextFormField(
            ...
            maxLines: 2,
            style: const TextStyle(fontSize: 13, color: AppColors.textPrimary),
            decoration: _dec(hint: 'Optional notes about this order'),
          ),
    ...
    return Column(
      ...
      children: [
        ...
        TextFormField(
          ...
          style: TextStyle(
            fontSize: 13,
            color: _addressCtrl.text.trim().isEmpty
        ...
        children: [
          ...
          ...List.generate(widget.lines.length, (i) {
            return _TableRow(
              ...
              onQtyChanged: () {
                setState(() {}); // refresh line totals inside table
                widget.onQtyChanged();
              },
            );
          }),
    ...
    return Container(
      ...
      child: const Row(
        children: [
          ...
          SizedBox(width: 36), // trash icon column
        ],
      ),
    );
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Footer — two action buttons (no cancel)
// ─────────────────────────────────────────────────────────────────────────────
...
class _Footer extends StatelessWidget {
  ...
  final VoidCallback? onSaveAndAdd; // null when editing
  ...
  Widget build(BuildContext context) {
    ...
    final createAndAddBtn = onSaveAndAdd != null
        ? OutlinedButton.icon(
