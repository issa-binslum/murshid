...
import 'purchase_invoice_detail_page.dart';
...
class _OrdersTabState extends ConsumerState<_OrdersTab>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ref.read(purchaseOrderProvider.notifier).load();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _q.isEmpty
        ...
        : state.items.where((o) {
            ...
            return o.supplierName.toLowerCase().contains(q) ||
      ...
      children: [
        ...
        if (filtered.isNotEmpty)
          Expanded(
            child: isWide
                ? _OrdersDesktopTable(
                    orders: filtered, canManage: widget.canManage)
                : _OrdersMobileList(
                    orders: filtered, canManage: widget.canManage),
          ),
      ],
...
class _InvoicesTabState extends ConsumerState<_InvoicesTab>
  ...
  Widget build(BuildContext context) {
    ...
    // Auto-reload when any payment mutation happens (create, update, or delete)
    ...
    final filtered = _q.isEmpty
        ...
        : state.items.where((inv) {
            final q = _q.toLowerCase();
            return inv.supplierName.toLowerCase().contains(q) ||
    ...
    return Column(
      children: [
        _TabHeader(
          ...
          canManage: widget.canManage,
          onAdd: () => Navigator.of(context).push(MaterialPageRoute(
            builder: (_) => const PurchaseInvoiceFormPage(),
            fullscreenDialog: true,
          )),
        ...
        if (!state.isLoading && filtered.isEmpty)
          _EmptyState(
            ...
            label: 'No purchase invoices yet',
            canManage: widget.canManage,
            onAdd: () => Navigator.of(context).push(MaterialPageRoute(
              builder: (_) => const PurchaseInvoiceFormPage(),
              fullscreenDialog: true,
        ...
        if (filtered.isNotEmpty)
          Expanded(
            child: isWide
                ? _InvoicesDesktopTable(
                    invoices: filtered, canManage: widget.canManage)
                : _InvoicesMobileList(
                    invoices: filtered, canManage: widget.canManage),
          ),
      ],
...
class _OrdersDesktopTable extends StatelessWidget {
  ...
  const _OrdersDesktopTable(
      {required this.orders, required this.canManage});
    ...
    return Column(
      children: [
        Container(
          ...
          child: const Row(
            children: [
              Expanded(flex: 30, child: _ColHead(label: 'Supplier')),
              Expanded(flex: 20, child: _ColHead(label: 'Date')),
              Expanded(
                  flex: 28,
                  child: _ColHead(label: 'Description')),
              Expanded(
                  flex: 18,
                  child: _ColHead(label: 'Total', right: true)),
              SizedBox(width: 16),
        ...
        Expanded(
          child: ListView.separated(
            ...
            itemBuilder: (_, i) => _OrdersDesktopRow(
                order: orders[i], canManage: canManage, index: i),
          ),
        ),
      ],
    );
    ...
    return Material(
      ...
      child: InkWell(
        ...
        onTap: () => Navigator.of(context).push(MaterialPageRoute(
          builder: (_) => PurchaseOrderDetailPage(order: order),
        )),
        child: Padding(
          padding: const EdgeInsets.symmetric(
      ...
      onSelected: (v) {
        if (v == 'edit') {
          Navigator.of(context).push(MaterialPageRoute(
            builder: (_) => PurchaseOrderFormPage(editing: order),
            fullscreenDialog: true,
          ));
        } else if (v == 'cancel') {
          ref.read(purchaseOrderProvider.notifier).cancel(order.id);
        }
      },
      itemBuilder: (_) => [
        const PopupMenuItem(
...
class _OrdersMobileRow extends ConsumerWidget {
  final PurchaseOrder order;
  final bool canManage;
  const _OrdersMobileRow(
      {required this.order, required this.canManage});
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 40,
              decoration: BoxDecoration(
                color: AppColors.iconPurchases.withAlpha(25),
                shape: BoxShape.circle,
              ),
              child: const Icon(Icons.shopping_cart_outlined,
                  size: 18, color: AppColors.iconPurchases),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Column(
    ...
    return Column(
      children: [
        Container(
          ...
          child: const Row(
            children: [
              Expanded(flex: 28, child: _ColHead(label: 'Supplier')),
              ...
              Expanded(
                  flex: 16,
                  child: _ColHead(label: 'Balance', right: true)),
              SizedBox(width: 16),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Material(
      ...
      child: InkWell(
        ...
        onTap: () => Navigator.of(context).push(MaterialPageRoute(
          builder: (_) =>
              PurchaseInvoiceDetailPage(invoice: invoice),
        )),
        child: Padding(
          padding: const EdgeInsets.symmetric(
              horizontal: 24, vertical: 14),
          child: Row(
            children: [
              Expanded(
                flex: 28,
                child: Text(invoice.supplierName,
                    style: const TextStyle(
                        fontSize: 14,
                        fontWeight: FontWeight.w500,
                        color: AppColors.textPrimary),
    ...
    return PopupMenuButton<String>(
      onSelected: (v) {
        if (v == 'edit') {
          Navigator.of(context).push(MaterialPageRoute(
            builder: (_) =>
                PurchaseInvoiceFormPage(editing: invoice),
            fullscreenDialog: true,
          ));
        } else if (v == 'cancel') {
          ref
...
class _InvoicesMobileList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListView.separated(
      ...
      separatorBuilder: (_, __) =>
          const Divider(height: 1, color: AppColors.border),
      itemBuilder: (_, i) => _InvoicesMobileRow(
          invoice: invoices[i], canManage: canManage),
    );
  }
}
