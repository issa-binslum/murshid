...
import '../../core/utils/purchase_order_pdf.dart';
...
class PurchaseOrderDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final live = ref.watch(purchaseOrderProvider).items.firstWhere(
          (o) => o.id == order.id,
          orElse: () => order,
        );
    final canManage =
        ...
        child: Column(
          ...
          children: [
            _PageHeader(
              ...
              onPdf: () async {
                final bytes = await buildPurchaseOrderPdf(live);
                if (context.mounted) {
                  await saveAndOpenPdf(
                      context, bytes, 'PurchaseOrder_$shortId.pdf');
                }
              },
              onEdit: () => Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => PurchaseOrderFormPage(editing: live),
                  fullscreenDialog: true,
                ),
              ),
              onCopyToInvoice: () => Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => PurchaseInvoiceFormPage(fromOrder: live),
                  fullscreenDialog: true,
                ),
              ),
              onDuplicateOrder: () => Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => PurchaseOrderFormPage(fromOrder: live),
                  fullscreenDialog: true,
                ),
              ),
  ...
  void _showCancelDialog(
      BuildContext context, WidgetRef ref, PurchaseOrder o) {
    showDialog<void>(
      context: context,
...
// ─────────────────────────────────────────────────────────────────────────────
// Unified order card — info + items table
// ─────────────────────────────────────────────────────────────────────────────
                ...
                children: [
                  Row(
                    children: [
                      ...
                      _StatusChip(isCancelled: isCancelled),
                    ],
                  ),
...
class _StatusChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final color =
        isCancelled ? AppColors.dangerText : const Color(0xFF3B6D11);
    final bg = isCancelled ? AppColors.dangerBg : const Color(0xFFEAF3DE);
    final label = isCancelled ? 'Cancelled' : 'Active';
    return Container(
    ...
    return PopupMenuButton<String>(
      ...
      itemBuilder: (_) => [
        const PopupMenuItem(
          ...
          child: Row(
            children: [
              Icon(Icons.description_outlined, size: 16, color: AppColors.textPrimary),
              SizedBox(width: 10),
              Text('Purchase Invoice', style: TextStyle(fontSize: 13, color: AppColors.textPrimary)),
            ],
          ),
        ),
        const PopupMenuItem(
          ...
          child: Row(
            children: [
              ...
              SizedBox(width: 10),
              Text('Duplicate Order', style: TextStyle(fontSize: 13, color: AppColors.textPrimary)),
            ],
          ),
        ),
      ],
            ...
            children: [
              ...
              RichText(
                text: TextSpan(
                  ...
                  children: [
                    const TextSpan(text: 'You are about to cancel order '),
                    TextSpan(
                      text: '#$shortId',
                      style: const TextStyle(
                          fontWeight: FontWeight.w700,
