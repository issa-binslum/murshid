...
class PurchaseInvoiceFormPage extends ConsumerStatefulWidget {
  final PurchaseInvoice? editing;
  final PurchaseOrder? fromOrder;
  final PurchaseInvoice? fromInvoice;
  const PurchaseInvoiceFormPage({super.key, this.editing, this.fromOrder, this.fromInvoice});
...
class _PurchaseInvoiceFormPageState
  ...
  void initState() {
    ...
    _supplierId = e?.supplierId ?? fo?.supplierId ?? fi?.supplierId;
  ...
  Future<bool> _persist() async {
    if (_isEdit) {
      return ref.read(purchaseInvoiceProvider.notifier).update(
            widget.editing!.id,
      ...
      return ref.read(purchaseInvoiceProvider.notifier).create(
            supplierId: _supplierId!,
            date: _date,
            items: _linePayload,
            description: _descCtrl.text.trim(),
  ...
  void _handleError() {
    final err =
        ref.read(purchaseInvoiceProvider).error ?? 'An error occurred.';
    ref.read(purchaseInvoiceProvider.notifier).clearError();
    setState(() {
      _saving = false;
      _error = err;
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: isWide
          ? _wideLayout(suppliers, inventory)
          : _narrowLayout(suppliers, inventory),
    );
  }
  ...
  Widget _narrowLayout(
      List<Supplier> suppliers, List<InventoryItem> inventory) {
    return Column(
      children: [
        _Header(
          title: _isEdit ? 'Edit Purchase Invoice' : 'New Purchase Invoice',
