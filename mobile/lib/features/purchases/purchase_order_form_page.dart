...
class PurchaseOrderFormPage extends ConsumerStatefulWidget {
  ...
  final PurchaseOrder? fromOrder;
  const PurchaseOrderFormPage({super.key, this.editing, this.fromOrder});
...
class _PurchaseOrderFormPageState
  ...
  void initState() {
    ...
    _supplierId = e?.supplierId ?? fo?.supplierId;
  ...
  void _onItemSelected(int i, String itemId, List<InventoryItem> inventory) {
    ...
    setState(() {
      ...
      _lines[i].unitPrice = inv.purchasePrice;
    });
  }
  ...
  Future<bool> _persist() async {
    if (_isEdit) {
      return ref.read(purchaseOrderProvider.notifier).update(
            widget.editing!.id,
      ...
      return ref.read(purchaseOrderProvider.notifier).create(
            supplierId: _supplierId!,
            date: _date,
            items: _linePayload,
            description: _descCtrl.text.trim(),
  ...
  void _handleError() {
    final err =
        ref.read(purchaseOrderProvider).error ?? 'An error occurred.';
  ...
  Widget build(BuildContext context) {
    final suppliers = ref.watch(supplierProvider).items;
    ...
    return Scaffold(
      ...
      body:
          isWide ? _wideLayout(suppliers, inventory) : _narrowLayout(suppliers, inventory),
    );
  }

  Widget _wideLayout(List<Supplier> suppliers, List<InventoryItem> inventory) {
    return Center(
      child: ConstrainedBox(
        ...
        child: Card(
          ...
          child: _cardBody(suppliers, inventory),
        ),
      ),
    );
  }

  Widget _narrowLayout(List<Supplier> suppliers, List<InventoryItem> inventory) {
    return Column(
      children: [
        _Header(
        ...
        Expanded(
          child: SingleChildScrollView(
            ...
            child: _formContent(suppliers, inventory),
          ),
        ),
        _Footer(
          saving: _saving,
  ...
  Widget _cardBody(List<Supplier> suppliers, List<InventoryItem> inventory) {
    return Column(
  ...
  Widget _formContent(List<Supplier> suppliers, List<InventoryItem> inventory) {
    return Form(
      key: _formKey,
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            value: _supplierId,
            decoration: _dec(hint: 'Select supplier'),
            isExpanded: true,
            style: const TextStyle(
