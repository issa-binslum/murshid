...
class _CustomersScreenState extends ConsumerState<CustomersScreen> {
  ...
  Future<void> _openForm(Customer? editing) async {
    await showDialog<bool>(
      context: context,
      builder: (_) => CustomerFormDialog(editing: editing),
    );
  }
  ...
  Future<void> _confirmDelete(Customer customer) async {
    await showDialog<void>(
      context: context,
      builder: (_) => _DeleteModal(customer: customer),
    );
  }
      ...
      child: Row(
        children: [
          Container(
            ...
            height: 38,
            decoration: BoxDecoration(
              color: AppColors.iconCustomers.withAlpha(20),
              borderRadius: BorderRadius.circular(8),
            ),
            child: const Icon(Icons.person_outline_rounded,
                size: 20, color: AppColors.iconCustomers),
          ),
...
class _DeleteModal extends ConsumerStatefulWidget {
  final Customer customer;
  const _DeleteModal({required this.customer});
...
class _DeleteModalState extends ConsumerState<_DeleteModal> {
  ...
  Future<void> _confirm() async {
    ...
    final ok =
        await ref.read(customerProvider.notifier).delete(widget.customer.id);
    if (!mounted) return;
    if (ok) {
